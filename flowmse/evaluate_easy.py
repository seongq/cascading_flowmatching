...
if __name__ == '__main__':
    ...
    parser.add_argument("--condition", type=str, choices=('mixture', 'self'), required=True)
            ...
            if condition == "mixture":
                ...
                if N_mid ==0:
                    if N ==1:
                        ...
                        ENHANCEMENT = x1 - model(x1,torch.ones(Y.shape[0], device=Y.device),Y)
                        
                        x1, _ = model.ode.prior_sampling(Y.shape,ENHANCEMENT)
                        CONDITION = 0.5 * ENHANCEMENT + 0.5 * Y
                        
                        xt = x1
                        timesteps = torch.linspace(reverse_starting_point, reverse_end_point, N-1, device=Y.device)
                        for i in range(len(timesteps)):
                            t = timesteps[i]
                            if i != len(timesteps) - 1:
                            ...
                            dt = -stepsize    
                            xt = xt + dt * model(xt, vec_t, CONDITION)
                    ...
                    xt = Y_prior.to(Y.device)
                    timesteps = torch.linspace(reverse_starting_point, reverse_end_point, N_mid, device=Y.device)
                    for i in range(len(timesteps)):
                        t = timesteps[i]
                        if i != len(timesteps) - 1:
                            stepsize = t - timesteps[i+1]
                    ...
                    raise(f"N_mid 크기가 {N_mid}이다. N_mid는 0보다 크거나 같은 정수")
