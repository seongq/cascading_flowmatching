...
if __name__ == '__main__':
     ...
     parser.add_argument("--batch_size", type=int, default=4,  help="During training take at least N_min reverse steps")
     parser.add_argument("--pre_ckpt", type=str,  help="Load ckpt")     
     parser.add_argument("--base_dir", type=str)
     parser.add_argument("--weight_shat",type=float, default=0.8)
     parser.add_argument("--N_epsilon", type=int, required=True)
    ...
    # Load score model
     ...
     save_dir_path = f"{model.ode.__class__.__name__}_SGMSE_CRP_{args.N_epsilon}_to_conditioned_weight_shat_{args.weight_shat}_dataset_{dataset}_{formatted_time}"
     if model.ode.__class__.__name__ == "FLOWMATCHING":
          logger = WandbLogger(project=f"{model.ode.__class__.__name__}_SGMSE_CRP_to_conditioned",  save_dir="logs", name=save_dir_path)
     ...
     trainer = pl.Trainer(  accelerator='gpu', strategy=DDPPlugin(find_unused_parameters=False), gpus=[2], auto_select_gpus=False, 
