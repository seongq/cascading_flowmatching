...
from flowmse.model import VFModel_Finetuning_SGMSE_CRP
...
if __name__ == '__main__':
     ...
     parser.add_argument("--pre_ckpt", type=str,  required=True, help="Load ckpt")     
     parser.add_argument("--base_dir", type=str)
     parser.add_argument("--weight_shat",type=float, default=0.8)
     parser.add_argument("--N_epsilon",type=int, default=5, required=True)
    ...
    # Load score model
     model = VFModel_Finetuning_SGMSE_CRP.load_from_checkpoint(
        ...
        batch_size=args.batch_size, N_epsilon= args.N_epsilon, num_workers=4, kwargs=dict(gpu=False)
    )
     ...
     save_dir_path = f"{model.ode.__class__.__name__}_SGMSE_CRP_N_eps_{args.N_epsilon}_dataset_{dataset}_{formatted_time}"
